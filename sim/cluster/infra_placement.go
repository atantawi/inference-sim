...
// Returns the list of instances that were successfully placed.
func (pm *PlacementManager) RetryPendingInstances() []pendingInstance {
	if len(pm.pendingInsts) == 0 {
		return nil
	}

	var nowPlaced []pendingInstance
	maxIter := len(pm.pendingInsts) // R19: at most one pass through pending list per call
	i := 0
	for i < len(pm.pendingInsts) && i < maxIter {
		p := pm.pendingInsts[i]
		_, _, err := pm.PlaceInstance(p.id, p.model, p.gpuType, p.tpDegree)
		if err == nil {
			nowPlaced = append(nowPlaced, p)
